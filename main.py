...
def add_person():
    ...
    person = Person(
        request.json["name"], request.json["ingreso"], request.json["salida"]
    )
    s = expenseTracker.find_one({"name": person.name})
    if not (s):
        person_id = expenseTracker.insert(
            {
                "name": person.name,
        ...
        output = {
            ...
            "ingreso": new_person["ingreso"],
            "salida": new_person["salida"],
            "saldo": new_person["saldo"],
        }
    else:
...
def add_expense():
    ...
    expense = Expense(
        request.json["expense_name"],
        request.json["desc"],
        request.json["costo"],
        request.json["fecha"],
        request.json["personas"],
        request.json["pagadores"],
    )
    ...
    for pagador in expense.pagadores:
        ...
        expenseTracker.persons.update_one(
            {"name": pagador["name"]}, {"$set": {"saldo": saldoAux}}
        )

    expense_id = expenseTracker.expenses.insert(
        {
            "name": expense.name,
            "desc": expense.desc,
            "costo": expense.costo,
            "fecha": expense.fecha,
            "pagadores": expense.pagadores,
            "personas": expense.personas,
        }
    )
    ...
    output = {
        ...
        "costo": new_expense["costo"],
        "fecha": new_expense["fecha"],
        "pagadores": new_expense["pagadores"],
        "personas": new_expense["personas"],
    }
    return output
...
def delete_one_expense(id):
    ...
    if e:
        for p in e["pagadores"]:
            ...
            expenseTracker.persons.update_one(
                {"name": p["name"]}, {"$set": {"saldo": saldoAux}}
            )
        output = ""
    else:
        abort(404, description="Expense not found")
